...
エンドポイント・タブ（Body / Headers / Query / Capture）・レスポンス。マウスでも操作できる。

| キー | |
| --- | --- |
| `Tab` / `Shift-Tab` | ペインを移る（当たっているペインは枠が光る） |
| `↑` `↓` / `k` `j` | 当たっているペインで選ぶ / スクロールする |
| `←` `→` | タブに当たっているときはタブを切り替える |
| `PgUp` `PgDn` / `g` `G` | ページ送り / 先頭・末尾へ |
| `Enter` | 送る（送信中は `Esc` か `Ctrl-C` で打ち切り） |
| `/` | 一覧を絞り込む |
| `e` | 当たっているペインを編集する |
| `T` | 定義まるごとを TOML で編集する |
| `w` / `E` | workspace / 環境を切り替える |
| `v` | 変数を一覧する（秘匿値は伏せたまま） |
| `d` | ダンプを書くかどうかを切り替える |
| `m` | マウスの捕捉を切る（端末のテキスト選択を戻したいとき） |
| `q` / `Ctrl-C` | 終わる |

編集器の中では `Ctrl-S` で保存、`Ctrl-F` で検索、`Esc` で破棄。保存は `ailo new` と
同じ検証を通るので、秘匿値を直接書くと止まる。**秘匿値が直接書かれた定義は、そもそも
編集器を開かない**（編集器には生の定義が入るので、開いた時点で画面に出てしまう）。
CLI で `ailo secret set` に預けてから開くこと。

変数は展開せずに出し、解決できない `{{名前}}` は赤で出る。定義に直接書かれた秘匿値は
画面上で伏せる。レスポンスはマスクを通したものだけを表示し、全文はいつもどおりダンプに残る。
**端末でないところでは起動せず**、`ailo ls` / `ailo run` を案内して落ちる。
